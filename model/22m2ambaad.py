...
class DWConv(nn.Module):
    def __init__(self, in_channels, out_channels, ksize, stride=1,padding=1):
        ...
        self.dconv = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=ksize,padding=padding,stride=stride, groups=in_channels),
...
class Jetter(nn.Module):
    def __init__(self, in_channels,radio=16):
        super().__init__()
    
        self.jetter = nn.Parameter(torch.randn(1,1,1,in_channels))
        self.l1 = nn.Linear(in_channels, in_channels//radio)
        self.n1 = nn.InstanceNorm2d(in_channels//radio)
        self.ac = nn.SiLU()
        self.l2 = nn.Linear(in_channels//radio, in_channels)
        self.n2 = nn.InstanceNorm2d(in_channels)
        self.dp = nn.Dropout(0.3)
    def forward(self, x):
        b,h,w,c = x.size()
        jetter = self.jetter.repeat(b,h,w,1)
        jetter = self.ac(self.n1(self.l1(jetter).permute(0,3,1,2))).permute(0,2,3,1)
        jetter = self.ac(self.n2(self.l2(jetter).permute(0,3,1,2))).permute(0,2,3,1)
        jetter = self.dp(jetter)
        return x + jetter
    
# import random
# def add_jitter(feature_tokens, scale, prob=1):
#     """
#     Args:
...
class ConvMambaBlock(nn.Module):
    def __init__(self,
        ...
        self.conv11 = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, kernel_size = 1, stride = 1,bias=False),nn.InstanceNorm2d(hidden_dim),nn.SiLU())
        self.conv33 = DWConv(hidden_dim, hidden_dim, 3)
        self.pool55 = nn.MaxPool2d(5, stride=1, padding=2)
        self.mlp = nn.Sequential(nn.Conv2d(hidden_dim*3, hidden_dim, kernel_size=1, stride=1),nn.InstanceNorm2d(hidden_dim),nn.SiLU())  
       
        self.hss_block = HSSBlock(hidden_dim=hidden_dim, drop_path=drop_path, norm_layer=norm_layer, attn_drop_rate=attn_drop_rate, d_state=d_state, size=size, scan_type=scan_type, num_direction=num_direction,**kwargs)
        self.cbam = CBAM(hidden_dim)
        self.jetter = Jetter(hidden_dim)        
    def forward(self, x):
        conv_input = x.permute(0, 3, 1, 2)
        y11 = self.conv11(conv_input)
        y33 = self.conv33(conv_input)
        y55 = self.pool55(conv_input)
        y_mul = torch.cat([y11, y33, y55], dim=1)
        mlp_out = self.mlp(y_mul)
        cbam_out = self.cbam(mlp_out).permute(0,2,3,1)
        hss_out = self.hss_block(mlp_out.permute(0,2,3,1))
        out = cbam_out + hss_out
        out = self.jetter(out)
...
class LSSModule_v2(nn.Module):
    def __init__(self,
            ...
            **kwargs):
            super().__init__()
            self.convMamba = nn.ModuleList([ConvMambaBlock(hidden_dim=hidden_dim, drop_path=drop_path, norm_layer=norm_layer, attn_drop_rate=attn_drop_rate, d_state=d_state, size=size, scan_type=scan_type, num_direction=num_direction,**kwargs) for _ in range(depth)])
            # self.final_conv  = nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1, stride=1)
    def forward(self, x):
        # x -  B H W C
        out = x
